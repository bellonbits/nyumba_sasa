...
import {
  ArrowLeft, Lock, Smartphone, Eye,
  FileText, Trash2, ChevronRight, ShieldCheck,
} from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F8]" style={{ paddingTop: "env(safe-area-inset-top, 0px)" }}>
      ...
      <div className="px-4 pt-5 pb-10 space-y-4">
        ...
        >
          <ShieldCheck className="text-white h-8 w-8 shrink-0" />
          <div>
          ...
          {ITEMS.map(({ icon: Icon, label, desc, href, color, iconColor, danger }, i) => (
            <Link key={label} href={href}>
              <div className={`flex items-center gap-3 px-4 py-4 hover:bg-gray-50 active:bg-gray-100 transition-colors ${i > 0 ? "border-t border-gray-100" : ""}`}>
                ...
                <ChevronRight className="text-gray-300 h-4 w-4 shrink-0" />
              </div>
            </Link>
          ))}
        </motion.div>
