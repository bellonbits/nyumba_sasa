...
import { ArrowLeftOutlined, UserOutlined, PhoneOutlined, CameraOutlined } from "@ant-design/icons";
...
export default function EditProfilePage() {
  ...
  if (!profile) return <div className="min-h-screen bg-white flex items-center justify-center"><div className="h-8 w-8 border-2 border-[#FF6A00] border-t-transparent rounded-full animate-spin" /></div>;
        ...
        <div className="relative">
          ...
          >
            <button className="absolute bottom-0 right-0 h-7 w-7 rounded-full bg-[#FF6A00] text-white flex items-center justify-center shadow-md">
              {uploading ? <div className="h-3 w-3 border border-white border-t-transparent rounded-full animate-spin" /> : <CameraOutlined style={{ fontSize: 12 }} />}
            </button>
          </Upload>
        </div>
      </div>
      ...
      <div className="px-4">
        <Form form={form} layout="vertical" onFinish={handleSave} size="large" requiredMark={false}>
          <Form.Item label={<span className="font-medium text-gray-700">Full name</span>} name="name" rules={[{ required: true }]}>
            <Input prefix={<UserOutlined className="text-gray-400" />} className="rounded-xl" />
          </Form.Item>
          <Form.Item label={<span className="font-medium text-gray-700">Phone number</span>} name="phone" rules={[{ required: true }]}>
            <Input prefix={<PhoneOutlined className="text-gray-400" />} className="rounded-xl" />
          </Form.Item>
          <Form.Item>
