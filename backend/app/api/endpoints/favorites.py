...
@router.post("/{listing_id}", response_model=APIResponse)
async def toggle_favorite(
    ...
    if existing:
        ...
        await db.commit()
        return APIResponse(data=None, message="Removed from favorites")
    else:
        new_fav = Favorite(
            id=str(uuid.uuid4()),
            user_id=user_id,
        ...
        return APIResponse(data=new_fav, message="Added to favorites")
